...
public class Visualization : MonoBehaviour, SGrabbable, Brushable
{
    public struct ReferenceAxis
    {
        public SAxis horizontal;
        public SAxis vertical;
        public SAxis depth;
        public SAxis horizontal2;
        public SAxis vertical2;
        public SAxis depth2;
    ...
    public List<SAxis> axes { get; internal set; }
    ...
    List<SPView> instantiatedViews = new List<SPView>();
    ...
    private void Axis_OnNormalize(float minNormalizer, float maxNormalizer)
    {
        ...
        if (viewType == ViewType.Histogram && histogramObject != null)
        {
            ...
            VisualisationFactory.UpdatetHistogramMesh(ScatterPlotSceneManager.Instance.dataObject,
            axes[0].axisId,
    ...
    public void UpdateVisualizations()
    {
        ...
        if (axes.Count == 1)
        {
            Staxes.Tuple<GameObject, Vector3[]> histT = VisualisationFactory.Instance.CreateBarHistogramView(ScatterPlotSceneManager.Instance.dataObject,
                axes[0].axisId,
                (int)HISTOGRAM_BIN_SIZE,
                false,
                1f,
        ...
        {
            SAxis axisV = axes[0].IsHorizontal ? axes[1] : axes[0];
            SAxis axisH = axes[0].IsHorizontal ? axes[0] : axes[1];

            referenceAxis.Clear();
            referenceAxis.horizontal = axisH;
            referenceAxis.vertical = axisV;

            Staxes.Tuple<GameObject, SPView> parallelT = VisualisationFactory.Instance.CreateSingle2DView(ScatterPlotSceneManager.Instance.dataObject, axes[0].axisId, axes[1].axisId, -1, VisualisationAttributes.Instance.LinkedAttribute,
            ...
            Staxes.Tuple<GameObject, SPView> scatter2DT = VisualisationFactory.Instance.CreateSingle2DView(ScatterPlotSceneManager.Instance.dataObject, axisH.axisId, axisV.axisId, -1, VisualisationAttributes.Instance.LinkedAttribute, MeshTopology.Points,
        ...
        else if (axes.Count == 3)
        {
            ...
            SAxis axisV = axes.FirstOrDefault(x => x.IsVertical);
            var horizontals = axes.Where(x => x != axisV).ToList();
            SAxis h0 = horizontals[0];
            SAxis h1 = horizontals[1];

            SAxis depth = null;
            SAxis horizontal = null;
            ...
            if (horizontal != null && axisV != null && depth != null)
            {
                ...
                Staxes.Tuple<GameObject, SPView> scatter3DT = VisualisationFactory.Instance.CreateSingle2DView(ScatterPlotSceneManager.Instance.dataObject,
        ...
        {
            SAxis axisV1 = axes[0].IsHorizontal ? axes[1] : axes[0];
            SAxis axisH1 = axes[0].IsHorizontal ? axes[0] : axes[1];

            SAxis axisV2 = axes[2].IsHorizontal ? axes[3] : axes[2];
            SAxis axisH2 = axes[2].IsHorizontal ? axes[2] : axes[3];
            ...
            var linkedView = VisualisationFactory.Instance.CreateLinked2DScatterplotsViews(ScatterPlotSceneManager.Instance.dataObject,
    ...
    void CalculateCorners1(SAxis axisA, SAxis axisB, SAxis axisC, ref Vector3 ftl, ref Vector3 ftr, ref Vector3 fbl, ref Vector3 fbr)
    {
    ...
    void CalculateCorners2(SAxis axisA, SAxis axisB, SAxis axisC, ref Vector3 ftl, ref Vector3 ftr, ref Vector3 fbl, ref Vector3 fbr)
    {
    ...
    void CalculateCorners4(SAxis axisA, SAxis axisB, SAxis axisC, ref Vector3 ftl, ref Vector3 ftr, ref Vector3 fbl, ref Vector3 fbr, ref Vector3 btl, ref Vector3 btr, ref Vector3 bbl, ref Vector3 bbr)
    {
        Vector3 up = axisA.transform.TransformVector(Vector3.up * 0.5f);
        Vector3 down = axisA.transform.TransformVector(Vector3.down * 0.5f);
    ...
    void CalculateCorners3(SAxis axisA, SAxis axisB, SAxis axisC, ref Vector3 ftl, ref Vector3 ftr, ref Vector3 fbl, ref Vector3 fbr)
    {
        ftl = axisA.transform.TransformPoint(Vector3.up * 0.5f);
    ...
    List<Vector3> Extents(params SAxis[] axes)
    {
    ...
    void LateUpdate()
    {
        ...
        switch (viewType)
        {
            ...
            case ViewType.Scatterplot2D:
                {
                    tag = "Scatterplot2D";
                    SAxis axisV = referenceAxis.vertical;
                    SAxis axisH = referenceAxis.horizontal;
            ...
            case ViewType.Scatterplot3D:
                {
                    ...
                    SAxis axisV = referenceAxis.vertical;
                    SAxis axisH = referenceAxis.horizontal;
                    SAxis axisD = referenceAxis.depth;
        ...
        if (isBrushing)
        {
            ...
            switch (viewType)
            {
                ...
                case ViewType.Scatterplot2D:
                    ...
                    SBrushingAndLinking.updateBrushedIndices(SBrushingAndLinking.BrushIndicesPointScatterplot(
                        verticesS2d,
                        SBrushingAndLinking.brushPosition,
                        SBrushingAndLinking.brushSize / 2f,
    ...
    public void AxesSnapToGrid(SAxis stillAxis, SAxis snapTo)
    {
    ...
    public Vector3?[] normaliser(Vector3?[] points)
    {
        switch (viewType)
        {
            case ViewType.Histogram:
                ...
                break;
            case ViewType.Scatterplot2D:
                SAxis axisV2D = referenceAxis.vertical;
                SAxis axisH2D = referenceAxis.horizontal;
                ...
                break;
            case ViewType.Scatterplot3D:
                SAxis axisH3D = referenceAxis.horizontal;
                SAxis axisV3D = referenceAxis.vertical;
                SAxis axisD3D = referenceAxis.depth;
    ...
    public Vector3?[] GetPoints()
    {
        ...
        switch (viewType)
        {
            case ViewType.Histogram:
                {
                    float[] x = ScatterPlotSceneManager.Instance.dataObject.getDimension(axes[0].axisId);
    ...
    public Vector3?[] get1DAxisCoordinates(int index)
    {
        ...
        if (viewType == ViewType.Scatterplot2D)
        {
            if (index <= (axes.Count - 1))
            {
                ...
                {
                    float[] x = ScatterPlotSceneManager.Instance.dataObject.getDimension(axes[index].axisId);
                    points = new Vector3?[x.Length];
                    for (int i = 0; i < x.Length; i++)
                    {
                        Vector3 pointToConvert = Vector3.zero;
                ...
                {
                    float[] y = ScatterPlotSceneManager.Instance.dataObject.getDimension(axes[index].axisId);
        ...
        else if (viewType == ViewType.Scatterplot3D)
        {
            if (index <= (axes.Count - 1))
            {
                ...
                {
                    float[] z = ScatterPlotSceneManager.Instance.dataObject.getDimension(axes[index].axisId);
    ...
    public void OnBrush(WandController controller, Vector3 hitPoint, bool is3D)
    {
        isBrushing = true;
        // swapToBrushing();
        SBrushingAndLinking.isBrushing = isBrushing;

        SBrushingAndLinking.brushPosition = hitPoint;
    }

...
public class ImAxesRecognizer : MonoBehaviour
{
    ...
    List<SAxis> A = new List<SAxis>();

    public List<Visualization> SP = new List<Visualization>();
    List<Visualization> PCP = new List<Visualization>();

    public List<SAxis> usedAxisIn3DSP = new List<SAxis>();
    public List<SAxis> usedAxisIn2DSP = new List<SAxis>();
    ...
    bool RSP1(SAxis a, SAxis b, SAxis c)
    {
        return
    ...
    bool RSP1_Distance(SAxis a, SAxis b, SAxis c)
    {
    ...
    List<SAxis> findSplomArrangement(List<SAxis> axes)
    {
        List<SAxis> list = new List<SAxis>();
    ...
    void ParseScene_V2()
    {
        ...
        A = ScatterPlotSceneManager.Instance.sceneAxes;
        // ==============================================================================
        // ======================== PASS0: PARSING SPs ==================================
        // ==============================================================================
        ...
        for (int i = 0; i < SP.Count; i++)
        {
...

            {
                ...
                List<SAxis> visuAxes = SP[i].axes;
                SAxis x3D = SP[i].ReferenceAxis1.horizontal;// visuAxes[0].IsHorizontal ? visuAxes[0] : visuAxes[1].IsHorizontal ? visuAxes[1] : visuAxes[2];
                SAxis y3D = SP[i].ReferenceAxis1.vertical; //visuAxes.First(x => x != x3D && x.IsVertical);// visuAxes[0] == x3D ? visuAxes[1] : visuAxes[2] == x3D? visuAxes[0] : visuAxes[1];
                SAxis z3D = SP[i].ReferenceAxis1.depth; //visuAxes.First(x => x != x3D && x != y3D);
        ...
        for (int sp3d = 0; sp3d < SPLOMS3D.Count; sp3d++)// (var splom3D in SPLOMS3D)
        {
            ...
            x3D = splom3D.XAxes1.Last(); // get the last X SAxis
            SAxis y3D = null;
            if(splom3D.YAxes1.Count>0)
            y3D = splom3D.YAxes1.Last(); // get the last Y SAxis

            SAxis z3D = null;
            if (splom3D.ZAxes1.Count>0)
            z3D = splom3D.ZAxes1.Last(); // get the last Z SAxis

            //look for a new SAxis
            SAxis newXAxis = null;
            SAxis newYAxis = null;
            SAxis newZAxis = null;
...
//                                + "SP2_SPLOM: " + SPLOMS2D.Count + "\n"
    ...
    bool AxisInScatterplotMaxtrix(SAxis axis)
    {
    ...
    public bool scatterplot2DContainsAxis(GameObject[] scatterplots2D, SAxis axis)
    {
    ...
    void connectedAxis(SAxis a, List<SAxis> A, ref List<SAxis> theConnectedAxes)
    {
        //List<SAxis> theConnectedAxes = new List<SAxis>();
